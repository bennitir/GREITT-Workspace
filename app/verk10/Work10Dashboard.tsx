        ...
        <section className="rounded-2xl border bg-white shadow-sm">
          <div className="flex flex-col gap-4 border-b px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex items-center gap-3 text-sm text-slate-600">
              <span className="rounded-full bg-amber-50 px-3 py-1.5 font-medium text-amber-800">{t.testDriveBadge}</span>
              <Link href="/verk" className="rounded-lg border px-3 py-2 font-semibold hover:bg-slate-50">
                {t.currentWork}
              </Link>
            </div>
          </div>
                  ...
                  <div className="sticky top-4 rounded-2xl border bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <Link href={`/verk10/${selectedWork.id}`} className="group block">
                    ...
                    <Link href={`/verk10/${selectedWork.id}`} className="mt-4 block rounded-xl bg-emerald-600 px-4 py-3 text-center text-sm font-bold text-white hover:bg-emerald-700">{t.openWork}</Link>
                  </div>
                ) : (
                ...
                {mainTab === "list" && (
                  <div className="mt-5 divide-y overflow-hidden rounded-xl border bg-white">
                    {data.workOrders.map((work) => (
                      <Link key={work.id} href={`/verk10/${work.id}`} className="flex w-full items-center justify-between gap-4 p-4 text-left hover:bg-slate-50">
          ...
          <div className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-2">
              {data.workOrders.slice(0, 6).map((work, index) => (
                <Link key={work.id} href={`/verk10/${work.id}`} className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left hover:bg-slate-50">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-semibold">
                <button disabled className="rounded-xl border px-3 py-3 text-slate-400">＋ {t.newWork}</button>
