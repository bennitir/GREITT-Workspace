        ...
        <header>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-slate-100 p-4">
              <p className="text-sm text-slate-600">{t.allWork}</p>
        ...
        <section className="mt-6">
          {workOrders.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center">
              <p className="text-lg font-bold text-slate-900">{t.noWork}</p>
              <p className="mt-2 text-slate-600">{t.noWorkHelp}</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">
                      {workStatusText(work.status, language)}
                    </span>
                  </div>

                  {work.address && (
                    <p className="mt-2 text-base text-slate-700">{work.address}</p>
                  )}

                  <div className="mt-3 flex items-center justify-between text-sm text-slate-500">
                    <span>
                      {t.priority}: {workPriorityText(work.priority, language)}
                    </span>
