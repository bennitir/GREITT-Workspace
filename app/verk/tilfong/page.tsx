            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              return (
                <article key={resource.id} className={`rounded-2xl border p-4 ${resource.isActive ? "bg-white" : "bg-slate-50 opacity-75"}`}>
                  ...
                  {access.canWrite && (
                    <details className="mt-3 rounded-xl border bg-slate-50 p-3">
                      ...
                      <form action={updateWorkResourceDetails} className="mt-3 grid gap-2 sm:grid-cols-2">
                        ...
                        <WorkResourceMeterFields
                          language={language}
                          kind={resource.kind as "MACHINE" | "VEHICLE" | "TOOL"}
                          initialMeterUnit={resource.meterUnit}
                          hasMeterHistory={resource.meterReadings.length > 0 || resource.meterValue !== null}
                          compact
                        />
                        {resource.kind === "MACHINE" ? (
                  ...
                  {(resource.kind !== "TOOL" || Boolean(resource.meterUnit) || resource.meterReadings.length > 0 || resource.meterValue !== null) ? (
