...
export default function ManualBookingEditor({
  ...
  const vatStatusText = useMemo(() => {
    if (vatRegistered === true) {
      return "Fyrirtækið er VSK-skráð.";
    }

    if (vatRegistered === false) {
      return "Fyrirtækið er ekki VSK-skráð. Innskattur og útskattur eru ekki bókaðir.";
    }

    return "VSK-skráningarstaða fyrirtækisins er ekki staðfest. Sjálfvirk VSK-bókun er óvirk.";
  }, [vatRegistered]);
  ...
  const availableAccounts = useMemo(() => {
    ...
    return accounts.filter(
      (account) => !isVatPostingAccount(account)
    );
      ...
      currentRows.map((row) => {
        ...
        if (field === "account") {
          ...
          if (
            matchedAccount &&
            vatRegistered !== true &&
            isVatPostingAccount(matchedAccount)
          ) {
            return {
              ...row,
              account: "",
  ...
  function setVatConfirmed(id: number, value: boolean) {
    ...
    setRows((currentRows) => {
      ...
      const vatAccount = accounts.find(
        (account) =>
          account.number === sourceRow.vatAccount
      );

      if (
        !vatAccount ||
        !isVatPostingAccount(vatAccount)
      ) {
        return currentRows.map((row) =>
          row.id === id
            ? {
            ...
            <div>
              ...
              >
                {[...availableAccounts]
                  .sort((a, b) => {
                    const aPreferred =
                      a.entryRole === row.entryRole
                        ? 0
                        : 1;

                    const bPreferred =
                      b.entryRole === row.entryRole
                        ? 0
                        : 1;
                ...
                row.entryRole !== "SYSTEM" && (
                  <div className="mt-1 text-xs text-gray-500">
                    ...
                    vatRegistered === true ? (
                      ...
                      false ? (
                      "Bókað án VSK-frádráttar."
                    ) : (
                      "VSK-meðferð bíður staðfestingar á VSK-skráningu fyrirtækisins."
                    )}
                  </div>
                )}
            </div>
